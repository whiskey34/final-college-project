...
class TransferLearning(object):
    ...
    def run(self):
        ...
        self.socketio.emit(
            self.event, "%s <b>__Start Transfer Learning__<b>" % self.get_time())
        self.socketio.sleep(1)
        ...
        self.model.add(Dense(self.dim, name='dense_last'))
        ...
        self.model.summary(
            print_fn=lambda x: self.socketio.emit(self.event, x))
        ...
        # train model
        self.socketio.emit(
            self.event, "%s training model started." % self.get_time())
        self.socketio.sleep(1)
        ...
        self.socketio.emit(
            self.event, "%s training model completed." % self.get_time())
        self.socketio.sleep(1)

        self.socketio.emit(
            self.event, "%s starting model otimization for inference." % self.get_time())
        self.socketio.sleep(1)
        ...
        self.socketio.emit(
            self.event, "%s saved model `tf_model/` created." % self.get_time())
        self.socketio.sleep(1)
        ...
        self.socketio.emit(self.event, "%s optimized model %s created." % (
            self.get_time(), optimized_model_name))
        self.socketio.sleep(1)
